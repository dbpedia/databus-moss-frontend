<script lang="ts">
	...
	export let unavailableMessage = 'This resource is temporarily unavailable.';
...
<div class="container">
	{#if data.status}
		<ContentGate status={data.status} {unavailableMessage} />
	{:else}
	...
	{#if data.mode === 'browse'}
		<div class="list-container">
			...
			{#if data.folders?.length}
				<FileList collection={data.folders} files={false} icon={browseIcon} />
			{/if}
			{#if data.files?.length}
				<FileList collection={data.files} />
			{/if}
		</div>
